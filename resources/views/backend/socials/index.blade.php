    ...
    <div class="container">
        <div class="row ">
            ...
            <div class="col-6 d-flex justify-content-end">
                ...
                <a href="{{ route('admin.socials.create') }}" class="btn btn-primary font-weight-bolder">
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px"
                            ...
                            <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                ...
                                <path
                                    ...
                                    fill="#000000" opasocialMedia="0.3" />
                            </g>
                        </svg>
                        <!--end::Svg Icon-->
                    </span>
        ...
        <div class="row">
            <div class="col-12">
                <table class="table table-bordered table-hover table-striped table-light yajra-datatable">
                    ...
                    <tbody>
                        @foreach ($socials as $k => $socialMedia)
                            <tr data-entry-id="{{ $socialMedia->id }}">
                                <td>{{ $loop->index+1 }}</td>
                                <td class="text-center">
                                    <img class="rounded" src="{{ asset('images/icon/'.$socialMedia->type).'.png' }}" width="90" height="60" alt="{{ $socialMedia->type }}">
                                </td>
                                <td class="text-center">{{ $socialMedia->type }}</td>
                                <td class="text-center">{{ $socialMedia->link }}</td>
                                <td class="text-center">
                                    <span class="switch switch-icon">
                                        <label>
                                            <input data-id="{{ $socialMedia->id }}" class="status-class" type="checkbox"
                                ...
                                <td class="text-center">
                                    <div style="display: flex" class="text-center justify-content-between">
                                        @ability('superAdmin', 'manage_socials,update_socials')
                                            <a href="{{ route('admin.socials.edit', $socialMedia->id) }}"
                                                class="edit btn btn-success btn-sm"><i class="fas fa-edit"></i>
                                            </a>
                                        @endability

                                        @ability('superAdmin', 'manage_socials,delete_socials')
                                            <a href="javascript:void(0)"
                                                onclick="
                                                    if (confirm('Are You Sure You Want To Delete This Record ?') )
                                                        { document.getElementById('record_delete_{{ $socialMedia->id }}').submit(); }
                                                    else
                                                        { return false; }"
                                    ...
                                    <form action="{{ route('admin.socials.destroy', $socialMedia->id) }}" method="post" id="record_delete_{{ $socialMedia->id }}" class="d-none">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </td>
