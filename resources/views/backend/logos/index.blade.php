    ...
    <div class="container">
        <div class="row ">
            <div class="col-6 d-flex text-left">
                <h1 class=" text-left">لوجو الموقع</h1>
            </div>
            <div class="col-6 d-flex justify-content-end">
                @ability('superAdmin', 'manage_Settings,show_logos')
                <a href="{{ route('admin.logos.create') }}" class="btn btn-primary font-weight-bolder">
                                ...
                                @ability('superAdmin', 'manage_Settings,show_logos')
                                    <td class="text-center">
                                        <span class="switch switch-icon">
                                            <label>
                                                <input data-id="{{ $logo->id }}" class="status-class" type="checkbox"
                                    ...
                                    <td class="text-center">
                                        <div style="display: flex" class="text-center justify-content-between">
                                            <a href="javascript:void(0)" onclick="
                                                                if (confirm('Are You Sure You Want To Delete This Record ?') )
                                                                    { document.getElementById('record_delete_{{ $logo->id }}').submit(); }
                                                                else
                                        ...
                                        <form action="{{ route('admin.logos.destroy', $logo->id) }}" method="post"
                                            id="record_delete_{{ $logo->id }}" class="d-none">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
        ...
        $(function () {
            $('.status-class').change(function() {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{ route('admin.logos.changeStatus') }}',
                    data: {
                        'status': status,
                        'cat_id': cat_id
                    },
