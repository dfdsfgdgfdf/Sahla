    ...
    <div class="container">
        <div class="row ">
            <div class="col-6 d-flex text-left">
                <h1 class=" text-left">E-Mails</h1>
            </div>
            <div class="col-6 d-flex justify-content-end">
                @ability('superAdmin', 'manage_emails,create_emails')
                <a href="{{ route('admin.emails.create') }}" class="btn btn-primary font-weight-bolder">
                    ...
                    <tbody>
                        @foreach ($emails as $k => $email)
                            <tr data-entry-id="{{ $email->id }}">
                                ...
                                <td class="text-center">{{ $email->type }}</td>
                                <td class="text-center">
                                    <span class="switch switch-icon">
                                        <label>
                                            <input data-id="{{ $email->id }}" class="status-class" type="checkbox"
                                ...
                                <td class="text-center">
                                    <div style="display: flex" class="text-center justify-content-between">
                                        @ability('superAdmin', 'manage_emails,update_emails')
                                            <a href="{{ route('admin.emails.edit', $email->id) }}"
                                                class="edit btn btn-success btn-sm"><i class="fas fa-edit"></i>
                                            </a>
                                        @endability

                                        @ability('superAdmin', 'manage_emails,delete_emails')
                                            <a href="javascript:void(0)"
                                                onclick="
                                                    if (confirm('Are You Sure You Want To Delete This Record ?') )
                                                        { document.getElementById('record_delete_{{ $email->id }}').submit(); }
                                                    else
                                                        { return false; }"
                                    ...
                                    <form action="{{ route('admin.emails.destroy', $email->id) }}" method="post" id="record_delete_{{ $email->id }}" class="d-none">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </td>
