    ...
    <div class="container">
        <div class="row ">
            <div class="col-6 d-flex text-left">
                <h1 class=" text-left">نصوص العناوين</h1>
            </div>
            <div class="col-6 d-flex justify-content-end">
                @ability('superAdmin', 'manage_Settings,show_page_title')
                <a href="{{ route('admin.page-titles.create') }}" class="btn btn-primary font-weight-bolder">
        ...
        <div class="row">
            <div class="col-12">
                <table class="table table-bordered table-hover table-striped table-light yajra-datatable">
                    ...
                    <tbody>
                        @foreach ($pageTitles as $k => $pageTitle)
                            <tr data-entry-id="{{ $pageTitle->id }}">
                                <td>{{ $loop->index + 1 }}</td>
                                ...
                                @ability('superAdmin', 'manage_Settings,show_page_title')
                                    <td class="text-center">
                                        <span class="switch switch-icon">
                                            <label>
                                                <input data-id="{{ $pageTitle->id }}" class="status-class" type="checkbox"
                                                    data-onstyle="success" data-offstyle="danger" data-toggle="toggle"
                                                    data-on="On" data-width="40" data-height="30" data-off="Off"
                                                    {{ $pageTitle->status == 1 ? 'checked' : '' }}>
                                                <span></span>
                                            </label>
                                        </span>
                                    </td>
                                    ...
                                    <td class="text-center">
                                        <div style="display: flex" class="text-center justify-content-between">
                                            <a href="{{ route('admin.page-titles.edit', $pageTitle->id) }}"
                                                class="edit btn btn-success btn-sm"><i class="fas fa-edit"></i>
                                            </a>

                                            <a href="javascript:void(0)" onclick="
                                                                if (confirm('Are You Sure You Want To Delete This Record ?') )
                                                                    { document.getElementById('record_delete_{{ $pageTitle->id }}').submit(); }
                                                                else
                                                                    { return false; }" class="btn btn-danger"><i
                                                    class="fa fa-trash"></i>
                                            </a>
                                        </div>
                                        <form action="{{ route('admin.page-titles.destroy', $pageTitle->id) }}" method="post"
                                            id="record_delete_{{ $pageTitle->id }}" class="d-none">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
        ...
        $(function () {
            $('.status-class').change(function() {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{ route('admin.page-titles.changeStatus') }}',
                    data: {
                        'status': status,
                        'cat_id': cat_id
                    },
