    ...
    <div class="container">
        <div class="row ">
            <div class="col-6 d-flex text-left">
                <h1 class=" text-left">التجار</h1>
            </div>
            <div class="col-6 d-flex justify-content-end">
                @ability('superAdmin', 'manage_merchants,create_merchants')
                <a href="{{ route('admin.merchants.create') }}" class="btn btn-primary font-weight-bolder">
        ...
        @include('backend.merchants.filter')
        ...
        <div class="row">
            <div class="col-12">
                <table class="table table-bordered table-hover table-striped table-light yajra-datatable">
                    ...
                    <tbody>
                        @foreach ($merchants as $k => $customer)
                            <tr data-entry-id="{{ $customer->id }}">
                                ...
                                <td class="text-center">
                                    <div style="display: flex" class="text-center justify-content-between">
                                        @ability('superAdmin', 'manage_merchants,update_merchants')
                                            <a href="{{ route('admin.merchants.edit', $customer->id) }}"
                                                class="edit btn btn-success btn-sm"><i class="fas fa-edit"></i>
                                            </a>
                                        @endability

                                        @ability('superAdmin', 'manage_merchants,delete_merchants')
                                    ...
                                    <form action="{{ route('admin.merchants.destroy', $customer->id) }}" method="post" id="record_delete_{{ $customer->id }}" class="d-none">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </td>
            ...
            var table = $('.yajra-datatable').DataTable({
                ...
                buttons: [{
                    ...
                    },
                    {
                        ...
                        text: 'Delete All',
                        url: "{{ route('admin.merchants.merchantsDestroyAll') }}",
        ...
        $(function () {
            $('.status-class').change(function() {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{ route('admin.merchants.changeStatus') }}',
                    data: {
                        'status': status,
                        'cat_id': cat_id
                    },
