    ...
    <div class="container">
        <div class="row ">
            <div class="col-6 d-flex text-left">
                <h1 class=" text-left">موقع الشركة</h1>
            </div>
            <div class="col-6 d-flex justify-content-end">
                @ability('superAdmin', 'manage_Settings,show_location')
                <a href="{{ route('admin.locations.create') }}" class="btn btn-primary font-weight-bolder">
                ...
                <table class="table table-bordered table-hover table-striped table-light yajra-datatable">
                    <thead class="table-dark ">
                        <tr class="text-light">
                            ...
                            <th class="text-light">وصف العنوان</th>
                            <th class="text-light">لينك Google Map</th>
                            @ability('superAdmin', 'manage_Settings,show_location')
                    ...
                    <tbody>
                        @foreach ($locations as $k => $location)
                            <tr data-entry-id="{{ $location->id }}">
                                ...
                                <td>
                                    <iframe src="{{ $location->location  }}" width="300" height="200" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
                                ...
                                @ability('superAdmin', 'manage_Settings,show_location')
                                    <td class="text-center">
                                        <span class="switch switch-icon">
                                            <label>
                                                <input data-id="{{ $location->id }}" class="status-class" type="checkbox"
                                    ...
                                    <td class="text-center">
                                        <div style="display: flex" class="text-center justify-content-between">
                                            ...
                                            <a href="javascript:void(0)" onclick="
                                                                if (confirm('Are You Sure You Want To Delete This Record ?') )
                                                                    { document.getElementById('record_delete_{{ $location->id }}').submit(); }
                                                                else
                                        ...
                                        <form action="{{ route('admin.locations.destroy', $location->id) }}" method="post"
                                            id="record_delete_{{ $location->id }}" class="d-none">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
