                    ...
                    <tbody>
                        @foreach ($productReviews as $k => $productReview)
                            <tr data-entry-id="{{ $productReview->id }}">
                                <td>{{ $loop->index+1 }}</td>
                                <td>
                                    <strong>{!! $productReview->user_id != '' ? $productReview->user->full_name : '' !!}</strong> <br>
                                ...
                                <td class="text-center">
                                    <span class="switch switch-icon">
                                        <label>
                                            <input data-id="{{ $productReview->id }}" class="status-class" type="checkbox"
                                ...
                                <td class="text-center">
                                            ...
                                            <a href="javascript:void(0)"
                                                onclick="
                                                    if (confirm('Are You Sure You Want To Delete This Record ?') )
                                                        { document.getElementById('record_delete_{{ $productReview->id }}').submit(); }
                                                    else
                                                        { return false; }"
                                    ...
                                    <form action="{{ route('admin.productReviews.destroy', $productReview->id) }}" method="post" id="record_delete_{{ $productReview->id }}" class="d-none">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </td>
        ...
        $(function () {
            $('.status-class').change(function() {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{ route('admin.productReviews.changeStatus') }}',
                    data: {
                        'status': status,
                        'cat_id': cat_id
                    },
            ...
            var table = $('.yajra-datatable').DataTable({
                ...
                buttons: [{
                    ...
                    },
                    {
                        ...
                        text: 'Delete All',
                        url: "{{ route('admin.productReviews.massDestroy') }}",
